...
namespace TiledSharp
{
    public class TmxMap : TmxDocument
    {
        ...
        public TmxMap(string filename, Func<string, Stream> readerFunc = null)
        {
            if (readerFunc == null)
            {
            ...
            {
                using (Stream inputStream = readerFunc(filename))
                {
                    Load(XDocument.Load(inputStream), readerFunc);
                }
            }
        }

        public TmxMap(Stream inputStream, Func<string, Stream> readerFunc = null)
        {
            Load(XDocument.Load(inputStream), readerFunc);
        }

        public TmxMap(XDocument xDoc, Func<string, Stream> readerFunc = null)
        {
            Load(xDoc, readerFunc);
        }
         
        private void Load(XDocument xDoc, Func<string, Stream> readerFunc = null)
        {
            ...
            foreach (var e in xMap.Elements("tileset"))
                Tilesets.Add(new TmxTileset(e, TmxDirectory, readerFunc));
            ...
            foreach (var e in xMap.Elements("imagelayer"))
                ImageLayers.Add(new TmxImageLayer(e, readerFunc == null ? TmxDirectory : string.Empty));

            Groups = new TmxList<TmxGroup>();
            foreach (var e in xMap.Elements("group"))
                Groups.Add(new TmxGroup(e, Width, Height, readerFunc == null ? TmxDirectory : string.Empty));
        }
    }
